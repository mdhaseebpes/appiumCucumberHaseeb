...
//        Boolean priceCheck = new ProductsPage().getProductPrice(title, price).equalsIgnoreCase(price);
        Assert.assertEquals("titleCheck = " + titleCheck + ", priceCheck = " + priceCheck,
    ...
    public void iShouldBeOnProductDetailsPageWithTitlePriceAndDescription(String title, String price, String description) throws Exception {
        ...
        Assert.assertEquals(productDetailsPage.getTitle(),title);
        Assert.assertEquals(productDetailsPage.getDesc(),description);
        Assert.assertEquals(productDetailsPage.getPrice(),price);
    }
}
